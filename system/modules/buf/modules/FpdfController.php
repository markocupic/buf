...
class FpdfController extends \System
{
    ...
    public function printDataSheet()
    {
        ...
        $pdf->SetFont('Arial', 'B', 11);
        $pdf->Cell(30, 6, utf8_decode('SchülerIn: '), 0, '', 'L');
        $pdf->SetFont('Arial', '', 11);
        $pdf->Cell(30, 6, utf8_decode(\StudentModel::getFullName(\Input::get('student'))), 0, '', 'L');
        $pdf->Ln();

        $pdf->SetFont('Arial', 'B', 11);
        $pdf->Cell(30, 6, 'Klasse: ', 0, '', 'L');
        $pdf->SetFont('Arial', '', 11);
        $pdf->Cell(30, 6, utf8_decode(\StudentModel::getClassnameFromStudentId(\Input::get('student'))), 0, '', 'L');
        $pdf->Ln();
        ...
        while ($objComment->next())
        {
            ...
            {
                $pdf->Ln();
                $pdf->Cell(190, 8, 'Kommentar von: ' . html_entity_decode(utf8_decode(\TeacherModel::findByPk($objComment->teacher)->firstname)) . ' ' . html_entity_decode(utf8_decode(\TeacherModel::findByPk($objComment->teacher)->lastname)) . ', ' . html_entity_decode(utf8_decode(\SubjectModel::findByPk($objComment->subject)->name)), 'B', '', 'L');
            }
            $pdf->Ln();
            $pdf->SetFont('Arial', 'B', 11);
            $pdf->Cell(190, 8, \Date::parse('Y-m-d', $objComment->dateOfCreation), '', '', 'L');
            $pdf->SetFont('Arial', '', 11);
            $pdf->Write(6, html_entity_decode(utf8_decode($objComment->comment)));
            $pdf->Ln();
            $prevId = $currentId;
