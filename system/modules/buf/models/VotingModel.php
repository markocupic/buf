              ...
              while ($objStudent->next())
              {
                     ...
                     if ($objVoting->numRows == 1)
                     {
                            ...
                            for ($i = 1; $i < 9; $i++)
                            {
                                   ...
                                   $array_name = "unterarray" . $i;
                                   $$array_name = array("value" => $skill, "deviation" => $deviation, "color" => $color, "average" => $intAverage, "date" => \Date::parse('d.m.Y', $tstamp), "tstamp" => $tstamp);
              ...
              if (intval($teacher) == $objUser->id)
              {
                     ...
                     if (preg_match('/^[0-4]{0,1}$/', $value))
                     {
                            ...
                            else
                            {
                                   if ($value > 0)
                                   {
                                          $set = array('student' => $student, 'teacher' => $teacher, 'subject' => $subject, 'tstamp' => time());
