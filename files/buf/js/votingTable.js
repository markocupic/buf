    ...
    EditTable = new Class({
        ...
        initialize: function (userIsOwner, isClassTeacher) {
            ...
            if (!this.userIsOwner) {
                ...
                $$('span.edit_icon').each(function (elImg) {
                    elImg.setProperty('title', 'keine Berechtigung');
                    elImg.addEvent('click', function (event) {
            ...
            if (this.userIsOwner) {                $$('.skillCell').addEvent('click', function(event){

                    if(!this.hasClass('active'))
                    {
                        event.stopPropagation();
                        self.edit(this, this.getProperty('data-id'), 'edit_row');
                    }
                });
            }
        ...
        deleteEntries: function (elLink, intValue, mode) {
            ...
            var req = new Request({
                url: '?isAjax=true&act=delete_row_or_col',
                method: "post",
                data: {
        ...
        edit: function (elementClick, intValue, mode) {
            var self = this;
            if (!this.userIsOwner) return;
            ...
            if (mode == 'edit_col') {
                $$('td.description.col_' + intValue).each(function (elCell) {
                    var bg = elCell.getStyle('background-image');
                    bg = bg.replace('bright','dark');
                    elCell.setStyle('background-image', bg);
                });
            }

            var submitButton = elementClick.getParent('tr').getElements('.submit_button')[0];
        ...
        updateTeachersDeviationTolerance: function (elRange) {

            var self = this;

            elRange.setProperty('title', 'Abweichungstoleranz: ' + elRange.value);

            var req = new Request({
                method: "post",
                url: '?isAjax=true&act=updateTeachersDeviationTolerance',
                data: {
                    REQUEST_TOKEN: self.request_token,
        ...
        resetTable: function () {
            ...
            $$('td.description').each(function (elCell) {
                i++;
                elCell.setStyles({'background-image': "url('files/buf/images/gd/images.php?bgcolor=bright&kriterium=" + i + "')"});
            });

            var req = new Request({
                method: 'post',
                url: '?isAjax=true&act=reset_table&class=' + self.class + '&subject=' + self.subject + '&teacher=' + self.teacher,
                data: {
                    REQUEST_TOKEN: self.request_token
                },
                onComplete: function (response) {
